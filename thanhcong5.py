...
class DonationPage(QWidget):
    ...
    def build_ui(self):
        ...
        success_row.setContentsMargins(12, 8, 0, 6)
        success_row.setSpacing(14)

        success_icon = QLabel("✔")
        success_icon.setFixedSize(58, 58)
        ...
        success_icon.setStyleSheet("""
            QLabel {
                ...
                color: #f2a9ca;
                background: #fff4f8;
                border: 3px solid #f2a9ca;
                border-radius: 29px;
            }
        """)
        success_row.addWidget(success_icon, alignment=Qt.AlignmentFlag.AlignTop)

        success_text = QLabel("Xác minh thành công\nBạn đã quyên góp thành công\nCảm ơn bạn!")
        success_text.setStyleSheet("font-size: 15px; color: #2c2328;")
        success_row.addWidget(success_text, alignment=Qt.AlignmentFlag.AlignVCenter)

        success_row.addStretch()
        donation_layout.addLayout(success_row)
...
if __name__ == "__main__":
    ...
    page5 = CampaignData(
        title="Chống hạn mặn Miền Tây (Thủy Tiên)",
        description="Lắp đặt các trạm lọc nước mặn thành nước ngọt công suất lớn để phục vụ sinh hoạt và tưới tiêu cho bà con vùng hạn mặn.",
        raised="1000000000",
        target="15000000000",
        percent="6.67%",
        activities="Lắp đặt máy lọc nước RO công nghiệp\nTặng bồn chứa nước sạch cho hộ nghèo\nVận chuyển nước ngọt cứu trợ khẩn cấp",
        scope="Đồng bằng sông Cửu Long",
        founded_date="2020-04-01",
        campaign_id="CC0012",
        gender="Nữ",
        full_name="Trần Thị Thủy Tiên",
        phone="0919222444",
        cccd="079185007777",
        address="Hồ Chí Minh",
        email="thuytien.singer@gmail.com",
        qr_path="qr5.png",
        avatar_path="avt5.png",
    )

    window = DonationPage(page5)
    window.show()
    sys.exit(app.exec())